...
*          lst_filter_range TYPE /iwbep/s_cod_select_option.
          ...
          lt_appconfig    TYPE TABLE OF /ODSMFE/CE_ServiceConfig.
...
*         SOC BY LMETTA
    SELECT recordnum,param_name,param_value,active,category
    FROM /odsmfe/tb_apcon WHERE active = 'X' INTO TABLE @lt_appconfig .
*         EOC BY LMETTA

    IF sy-subrc EQ 0.

      SORT lt_appconfig BY appstoreid.

      ex_response_data[] = lt_appconfig[].
...
* -----------------------------------------------------------------------*
    ...
    DATA: lv_fnam TYPE C LENGTH 30,
          lv_fval TYPE C LENGTH 30.
